        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <div class="row justify-content-center">
                    ...
                    <form class="user py-3 col-md-8 col-sm-8" method="post" action="{{ route('menu.edit', $menu) }}"
                        enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        <div class="text-center">
                            <h1 class="h4 text-gray-900 mb-4">Edit Menu : {{ $menu->nama }}</h1>
                        </div>
                        ...
                        <div class="mb-3">
                            <div class="input-group @error('nama') is-invalid @enderror">
                                ...
                                <input type="text" name="nama" value="{{ old('nama') ?? $menu->nama }}" id="nama"
                                ...
                                <select name="jenisMenu" class="custom-select @error('jenisMenu') is-invalid @enderror"
                                    ...
                                    @foreach ($jenisMenu as $data)
                                        <option value="{{ $data->id }}"
                                            {{ old('jenisMenu') == $data->id || $data->id == $menu->jenis_menu_id ? 'selected' : '' }}>
                                            {{ $data->nama }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-3">
                            <div class="input-group @error('harga') is-invalid @enderror">
                                ...
                                <input type="number" name="harga" value="{{ old('harga') ?? $menu->harga }}" id="harga"
                        ...
                        <div class="mb-3">
                            <textarea name="keterangan" class="form-control @error('keterangan') is-invalid @enderror"
                                ...
                                rows="5">{{ old('keterangan') ?? $menu->keterangan }}</textarea>
                            @error('keterangan')
                                <div class="invalid-feedback mb-2">{{ $message }}</div>
                            @enderror
                        </div>
